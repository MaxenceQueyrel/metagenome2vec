...
class FastDnaEmbed(Read2Vec):
    def __init__(self, path_read2genome, spark, path_tmp_folder):
        ...
        assert (
            path_read2genome is not None
            and spark is not None
            and path_tmp_folder is not None
    ...
    def read2vec(self, X, col_name="read", drop_col_name=True):
        ...
        subprocess.check_output(
            "%s/fastdna print-word-vectors %s < %s > %s"
            % (self.path_fastDNA, self.path_read2genome, read_name, emb_name),
            shell=True,
        )
        ...
        subprocess.call(
            "paste -d '%s' %s %s > %s" % (sep, read_name, emb_name, tmp_name),
            shell=True,
        )
        ...
        schema = T.StructType(
            list(X.schema)
            + [T.StructField(str(i), T.DoubleType(), True) for i in range(n_dim)]
        )
        # remove last embeddings always null, repartition the dataframe and persist
        X = (
            self.spark.read.csv(tmp_name, sep=sep, schema=schema, header=True)
            .drop(str(n_dim - 1))
            .repartition(nun_partitions)
            .persist()
        )
        X.count()
