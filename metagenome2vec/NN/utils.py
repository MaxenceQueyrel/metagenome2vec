...
from metagenome2vec.NN.deepsets import (
    Phi,
    Rho,
    DeepSets,
    score as score_deepsets,
    fit as fit_deepsets,
    table_prediction,
)
from metagenome2vec.NN.vae import VAE, AE, fit as fit_ae, evaluate as evaluate_ae
from metagenome2vec.NN.snn import (
    SiameseNetwork,
    fit as fit_snn,
...
def ray_hyperparameter_search(
    ...
    ray.init(
        num_cpus=np.int(np.ceil(D_resource["cpu"] * D_resource["worker"])),
    ...
    parameters = [
        ...
        {"name": clip, "type": "fixed", "value": -1.0},
    ]
    if model_type == "deepsets":
        ...
        parameters.append(
            {
                ...
                "type": "choice",
                "values": ["nn.ReLU", "nn.LeakyReLU"],
                "value_type": "str",
            }
        )
        parameters.append(
            {
                "name": n_layer_before_flatten,
                "type": "choice",
                "values": [2, 3, 4],
                "value_type": "int",
            }
    ...
    train_evaluate = train_evaluate_factory(
        X=X,
        y_=y_,
    ...
    analyse.dataframe().to_csv(
        os.path.join(path_model, "tuning_results.csv"), index=False
    )
...
def cross_val_score(
    ...
    for i, (X_train, X_valid, y_train, y_valid) in enumerate(
        tqdm(train_test_split_mil(X, y_, n_splits=cv, test_size=test_size), total=cv)
    ):
        params_loader = {"batch_size": params[batch_size], "shuffle": True}
        # Create the model
        if model_type == "deepsets":
            assert (
                output_size is not None
            ), "output_size must be define for deepsets model"
            phi = Phi(
                input_dim, params[hidden_init_phi], params[n_layer_phi], params[dropout]
            )
            rho = Rho(
                phi.last_hidden_size,
                params[hidden_init_rho],
                params[n_layer_rho],
                params[dropout],
                output_size,
            )
            ...
            params_loader["collate_fn"] = lambda x: collate_fn(
                x, device, batch_format=False
            )
        ...
        if model_type == "deepsets":
            ...
            if best_acc < acc:
                ...
                if params[mil_layer] == "attention":
                    ...
                    table_prediction(model, dataset_train).to_csv(
                        os.path.join(path_model_parent, "train_best_model.csv"),
                        index=False,
                    )
                    table_prediction(model, dataset_valid).to_csv(
                        os.path.join(path_model_parent, "valid_best_model.csv"),
                        index=False,
                    )
            if is_optimization:
                return np.mean(A_acc_valid), np.std(A_acc_valid)
            return {
                "test_accuracy": A_acc_valid,
                "fit_time": A_fit_time,
                "score_time": A_score_time,
                "test_accuracy": A_acc_valid,
                "train_accuracy": A_acc_train,
                "test_f1": A_f1_valid,
                "train_f1": A_f1_train,
                "test_precision": A_pre_valid,
                "train_precision": A_pre_train,
                "test_recall": A_rec_valid,
                "train_recall": A_rec_train,
                "test_roc_auc": A_auc_valid,
                "train_roc_auc": A_auc_train,
            }
        # Auto Encoder
        elif model_type == "vae" or model_type == "ae":
            fit_ae(
