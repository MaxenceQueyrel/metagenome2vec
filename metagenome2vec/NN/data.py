...
class MetagenomeNNDataset(Dataset):
    def __init__(self, X, y_):
        ...
        self.labels = self.data.drop_duplicates(subset=[id_subject_name])[
            group_name
        ].to_numpy()
        self.IDs = pd.unique(self.data[id_subject_name])
        self.D_data = {
            id_subject: self.data[self.data[id_subject_name] == id_subject]
            .drop([group_name, genome_name, id_subject_name], axis=1)
            .to_numpy()
            for id_subject in self.IDs
        }
        self.D_genome = {
            id_subject: self.data[self.data[id_subject_name] == id_subject][
                genome_name
...
def train_test_split_mil(X, y_, n_splits=1, test_size=0.2):
    ...
    X_uniq = X.drop_duplicates(subset=[id_subject_name, group_name])[
        [id_subject_name, group_name]
    ]
    ...
    for id_train, id_valid in sss.split(X_uniq[id_subject_name], X_uniq[group_name]):
        ...
        X_train, X_valid, y_train, y_valid = (
            X[id_train].reset_index(drop=True).drop(group_name, axis=1),
...
def load_matrix_for_learning(
    ...
    metadata = pd.read_csv(path_metadata, delimiter=",")[
        [id_fasta_name, group_name, id_subject_name]
    ].astype(str)
    ...
    if count_name in X.columns.values:  # Case MIL
        ...
        X = X.groupby(
            by=[id_subject_name, group_name, genome_name], as_index=False
        ).sum()
    else:  # case SIL
        if is_bok:
...
def load_several_matrix_for_learning(
    path_data, path_metadata, disease, model_type=None
):
    cpt_class = 2
    X = y_ = None
    for path_d, path_m, d in zip(
        path_data.split(","), path_metadata.split(","), disease.split(",")
    ):
        X_tmp, y_tmp = load_matrix_for_learning(
            path_d, path_m, d, model_type=model_type
        )
        if X is None:
            X, y_ = X_tmp, y_tmp
        else:
...
def mil_data_processing(X, y_, genomes=None):
    ...
    X_gb = (
        X[[id_subject_name, group_name, genome_name]]
        .groupby([id_subject_name, group_name])
        .agg({genome_name: lambda x: list(x)})
        .reset_index()
    )
    ...
    X_gb = pd.concat(
        [
            X_gb,
            pd.DataFrame(
                np.zeros((X_gb.shape[0], n_feature)),
                columns=[count_name] + [str(x) for x in range(n_feature - 1)],
            ),
        ],
        axis=1,
    )
    ...
    X = pd.concat(
        [
            X_metadata,
            X.groupby(id_subject_name).apply(lambda x: x / x[count_name].sum()),
        ],
        axis=1,
    )
