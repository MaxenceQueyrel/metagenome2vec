...
def transform(spark, k, path_data, df_metadata, overwrite):
    ...
    idx_to_del = [
        i
        for i in range(4**k)
        if transformation_ADN.int_to_kmer(i, k)[0] in ["G", "T"]
    ]
    m = 0
    for metagenome in df_metadata[id_fasta_name]:
        if not os.path.exists(os.path.join(path_data, metagenome)):
            logging.info(
                "Iteration %s, Metagenome : %s doesn't exist" % (m, metagenome)
            )
            continue
        logging.info("Iteration %s, Metagenome : %s" % (m, metagenome))
        kmer_count = spark.read.parquet(os.path.join(path_data, metagenome)).select(
            kmer_name, "count"
        )
