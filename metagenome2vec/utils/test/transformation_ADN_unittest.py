...
class TestStructurationSequence(unittest.TestCase):
    def test_create_context(self):
        ...
        real_res2 = [("AA", "CT"), ("AA", "CG"), ("TC", "AC"), ("TC", "GG")]
        real_res3 = [
            ("AA", "CTC"),
            ("AA", "GGA"),
            ("AC", "TCG"),
        ...
        self.assertListEqual(
            sorted(create_context(s1, k=2, kc=2, w=2)), sorted(real_res1)
        )
        self.assertListEqual(
            sorted(create_context(s2, k=2, kc=2, w=2, s=3)), sorted(real_res2)
        )
        self.assertListEqual(
            sorted(create_context(s3, k=2, kc=3, w=2)), sorted(real_res3)
        )
        self.assertListEqual(
            sorted(create_context_one(s3, k=2, kc=3, w=2)), sorted(real_res3_bis)
        )
        self.assertListEqual(
            sorted(create_context_reverse_one(s4, k=2, kc=2, w=2)), sorted(real_res4)
        )
        self.assertListEqual(
            sorted(create_context_reverse(s4, k=2, kc=2, w=2)), sorted(real_res5)
        )
    ...
    def test_cut_word(self):
        ...
        self.assertEqual(
            cut_word_one(s1, 2), [("GT", 1), ("TA", 1), ("AT", 1), ("TA", 1), ("AG", 1)]
        )
        self.assertEqual(
            cut_word_one(s2, 3, 2), [("AAT", 1), ("TTG", 1), ("GCG", 1), ("GCC", 1)]
        )
