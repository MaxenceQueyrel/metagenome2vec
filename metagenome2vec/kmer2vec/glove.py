...
if __name__ == "__main__":
    ...
    param_glove = "embSize_%s_nStep_%s_xMax_%s_learRate_%s" % (
        embedding_size,
        n_steps,
        x_max,
        learning_rate,
    ...
    subprocess.call(
        [
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "glove.sh"),
            file_word_count,
            file_cooccurrence,
            str(embedding_size),
            str(n_steps),
    ...
    subprocess.call(
        ["rm", os.path.join(path_save, "tmp.txt"), os.path.join(path_save, "tmp.bin")]
    )
