...
if __name__ == "__main__":
    ...
    param_fastext = "embSize_%s_nStep_%s_learRate_%s" % (
        embedding_size,
        n_steps,
        learning_rate,
    )
    ...
    subprocess.call(
        [
            os.path.join(path_fasttext, "fasttext"),
            "skipgram",
            "-input",
            path_data,
            "-output",
            os.path.join(path_save, "vectors"),
            "-lr",
            str(learning_rate),
            "-dim",
            str(embedding_size),
    ...
    df = pd.read_csv(
        os.path.join(path_save, "vectors.vec"), sep=" ", header=None, skiprows=1
    )
