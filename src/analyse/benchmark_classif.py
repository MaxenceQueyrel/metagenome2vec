...
from sklearn.model_selection import StratifiedShuffleSplit, RandomizedSearchCV, cross_validate
...
def benchmark(X, y_, dataset_name, n_splits, test_size, n_iter, computation_type=None):
    if computation_type is not None:
        X = X / X.sum(axis=0)
        transfo = clr if computation_type == "clr" else alr if computation_type == "alr" else ilr
        X = transfo(multiplicative_replacement(X))

    scalar = StandardScaler()
    inner_cv = StratifiedShuffleSplit(n_splits=n_splits, test_size=test_size, random_state=SEED)
    outer_cv = StratifiedShuffleSplit(n_splits=n_splits, test_size=test_size, random_state=SEED)

    rf = Pipeline([('transformer', scalar), ('estimator', RandomForestClassifier())])
    gb = Pipeline([('transformer', scalar), ('estimator', GradientBoostingClassifier())])
    ab = Pipeline([('transformer', scalar), ('estimator', AdaBoostClassifier())])
    svm = Pipeline([('transformer', scalar), ('estimator', SVC(max_iter=200, gamma="scale"))])

    D_model = {rf: {"estimator__n_estimators": np.arange(3, 30, 1),
                    "estimator__max_features": ["auto", "log2", None],
                    "estimator__max_depth": [None, 3, 6, 9]},
               gb: {"estimator__learning_rate": np.random.uniform(0, 2, 30),
                    "estimator__max_features": ["auto", "log2", None],
                    "estimator__n_estimators": np.arange(3, 30, 1),
                    "estimator__max_depth": np.arange(1, 5, 1)},
               ab: {"estimator__n_estimators": np.arange(3, 70, 1),
                    "estimator__learning_rate": np.random.uniform(0, 2, 30)},
               svm: {"estimator__C": np.random.randint(1, 50, 10),
    ...
    for model, parameters in D_model.items():
        ...
        scores = cross_validate(random_search, X, y_,
                                scoring=scoring, cv=outer_cv, return_train_score=True, n_jobs=n_cpus)
        classifier = str(model[1].__class__).replace('>', '').replace("'", "").split('.')[-1]
        data_manager.write_file_res_benchmarck_classif(path_save, dataset_name, classifier, scores)
...
class ResMetricCV:
    ...
    def update(self, acc, pre, rec, f1, auc, accv, prev, recv, f1v, aucv, fit_time, score_time):
    ...
    def get_scores(self):
        scores = {"fit_time": self.A_fit_time, "score_time": self.A_score_time,
                  "test_accuracy": self.A_acc_valid, "train_accuracy": self.A_acc_train,
                  "test_f1": self.A_f1_valid, "train_f1": self.A_f1_train,
                  "test_precision": self.A_pre_valid, "train_precision": self.A_pre_train,
                  "test_recall": self.A_rec_valid, "train_recall": self.A_rec_train,
                  "test_roc_auc": self.A_auc_valid, "train_roc_auc": self.A_auc_train}
        return scores
...
def benchmark_with_NN(X, y_, dataset_name, n_splits, test_size, n_iter):
    ...
    for i, (X_train, X_valid, y_train, y_valid) in enumerate(
            tqdm(vae.train_test_split(X, y_, col_features, n_splits=n_splits,
                                      test_size=test_size), total=n_splits)):
        ...
        params_loader = {'batch_size': nn_parameters[vae.batch_size],
                         'shuffle': True}
        loader_train = vae.DataLoader(dataset_train, **params_loader)
        loader_valid = vae.DataLoader(dataset_valid, **params_loader)

        auto_encoder = NN(nn_parameters[vae.input_dim], hidden_dim=nn_parameters[vae.hidden_dim],
                          n_layer_before_flatten=nn_parameters[vae.n_layer_before_flatten],
                          n_layer_after_flatten=nn_parameters[vae.n_layer_after_flatten], device=device).to(device)

        if tuning:
            optimizer = vae.optim.Adam(auto_encoder.parameters(), lr=nn_parameters[vae.learning_rate],
                                       weight_decay=nn_parameters[vae.weight_decay])
            # fitting
            fit(auto_encoder, loader_train, loader_valid, optimizer, nn_parameters[vae.n_epoch],
                nn_parameters[vae.clip], path_model=path_model, name_model=".checkpoint.pt")
            auto_encoder.eval()
        else:
            auto_encoder.load_state_dict(torch.load(os.path.join(path_model, vae.vae_name)))
            auto_encoder.eval()

        if fine_tuning:
            n_epoch = 30
            fine_tune_auto_encoder = vae.FineTuner(auto_encoder, n_output).to(device)
            optimizer = vae.optim.Adam(fine_tune_auto_encoder.parameters(), lr=nn_parameters[vae.learning_rate],
                                       weight_decay=nn_parameters[vae.weight_decay])
            vae.fit(fine_tune_auto_encoder, loader_train, loader_valid, optimizer, n_epoch, path_model=path_model)
            auto_encoder.eval()

        # Transformed X to embeddings
        X_train_transformed, y_train_transformed = auto_encoder.create_matrix_embeddings(loader_train, add_abundance)
        X_valid_transformed, y_valid_transformed = auto_encoder.create_matrix_embeddings(loader_valid, add_abundance)
        ...
        index_cv = np.concatenate(
            [np.full(y_train_transformed.shape, -1), np.full(y_valid_transformed.shape, 0)])
        ...
        for model, parameters in D_model.items():
            ...
            model[0].set_params(
                **{param.replace("estimator__", ""): val for param, val in random_search.best_params_.items()})
            ...
            # Scoring
            acc, pre, rec, f1, auc = score(y_train_transformed, y_pred_train, y_prob_train)
            accv, prev, recv, f1v, aucv = score(y_valid_transformed, y_pred_valid, y_prob_valid)
            fit_time = time_transformation_with_vae + time_training_classifier
            score_parameters = [acc, pre, rec, f1, auc, accv, prev, recv, f1v, aucv, fit_time, time_prediction_classifier]
...
if __name__ == "__main__":
    ...
    if path_model is None:
        X, y_ = data_manager.load_matrix_for_learning(path_data, path_metadata, disease, is_bok)
